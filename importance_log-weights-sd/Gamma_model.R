...
y <- as.vector(read.table("Gamma_data.txt")$V1)
T <- length(y)
gam <- 0.0 # allows using SV model AR(1) spec
delta <- 0.98
v <- 0.22
tau <- 0.13
beta <- 2.8
...
gg <- function(y,x){
  return(dgamma(y,shape=1/tau,scale=beta*tau*exp(x),log=TRUE))
}
...
for(iter in 1:J){
  ...
  plot(beta*exp(xx[1,]),type="l")
  for(i in 2:R) lines(beta*exp(xx[i,]),col=i)
  points(y)
  # regression pass
  lchi <- rep(0.0,R)
  for(t in T:1){
...
L <- CIP_TriDiagChol_const1n(T, (1.0/tau)+(1.0/vSq), (1.0/tau)+(1.0+deltaSq)/vSq, -delta/vSq)
...
for(k in 1:K){
  ...
  Gdiag <- y/(tau*beta*exp(x)) + c(1.0/(v^2),rep((1.0+delta^2)/(v^2),T-2),1.0/(v^2))
  Godiag <- rep(-delta/(v^2),T-1)
  L <- CIP_TriDiagChol(Gdiag,Godiag) 
  ngrad <- 1.0/tau - (1.0/(beta*tau))*y*exp(-x)
